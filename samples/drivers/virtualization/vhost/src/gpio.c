		...
		for (uint32_t s = 0; s < riov.used; s++) {
			...
			switch (req.type) {
				...
				case VIRTIO_GPIO_MSG_GET_DIRECTION:
					...
					printk("GET DIRE, gpio_pin_get_config: %d\n", ret);
					if (ret < 0) {
				...
				case VIRTIO_GPIO_MSG_SET_DIRECTION:
					// Preceding 'if' statement handling the 'default' scenario 
					...
					ret = gpio_pin_get_config(dev, req.gpio, &flags);
					printk("SET DIRE, gpio_pin_get_config: %d\n", ret);
					if (ret < 0) {
						LOG_ERR("gpio_pin_get_config failed: %d", ret);
						resp.status = VIRTIO_GPIO_STATUS_ERR;
						resp.value = 0;
					...
					// The default case is handled by the preceding 'if' statement.
					printk("Set direction (req.value): %d\n", req.value);
					switch (req.value) {
